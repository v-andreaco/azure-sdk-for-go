...
Write-Host "##[command] Executing 'go test -run "^Test" -race -v -coverprofile coverage.txt ./...' in sdk/$serviceDirectory"

go test -run "^Test" -race -v -coverprofile coverage.txt ./... | Tee-Object -FilePath outfile.txt
if ($LASTEXITCODE) {
    exit $LASTEXITCODE
}
